...
def load_institute_view_docs(root,institute_id):
    # window = Tk()
    ...
    student_id = uid_entry.get()
    def search_button():
        if(len(student_id)==13):
            load_institute_view(institute_id,student_id)
        else:
            messagebox.showerror("Incorrect Student_id", "enter valid student_id")
...
if __name__ == '__main__':
    root = tk.Tk()
    load_institute_view_docs(root,"I138615553989")