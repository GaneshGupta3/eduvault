...
class Document:
    ...
    def store_file_in_database(self, filename, file_type, file_name_to_store, file_category, file_size_in_bytes, file_data):
        """Store a file in the MySQL database."""
        try:
            dbobj = db()
            mydb, cursor = dbobj.dbconnect("documents")
            params = (self.uid, file_name_to_store, file_type, file_data, file_size_in_bytes, file_category)
            insert_query = "INSERT INTO files (uid, file_name, file_type, file_data, file_size, category) VALUES (%s, %s, %s, %s, %s, %s)"
            cursor.execute(insert_query, params)
            mydb.commit()
            mydb.close()
    ...
    def determine_decrypted_file_type(self,file_type):
    ...
    def store(self, raw_filename, file_name_to_store, file_category, file_size_in_bytes):
        file_name_to_store = file_name_to_store
        file_category = file_category
        file_size_in_bytes = file_size_in_bytes
        
        file_type = self.determine_file_type(raw_filename)

        try:
            with open(raw_filename, "rb") as file:
                file_data = file.read()

            self.store_file_in_database(raw_filename, file_type, file_name_to_store, file_category, file_size_in_bytes, file_data)
        except FileNotFoundError:
    ...
    def retrieve(self, file_id):
        ...
        if row:
            ...
            try:
                ...
                file_extension = self.determine_decrypted_file_type(file_type)

                # Open the temporary file with the default application
                self.open_file_with_default_program(file_data, file_type, temp_filename + file_extension)
            except Exception as e:
